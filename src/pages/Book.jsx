...
const Book = ({ setShowSessionTimer }) => {
    const { user, userRatedBooks } = useAuth();
    ...
    const bookId = path.split("/")[2]; // Get book id 
        ...
        const fetchBook = async () => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    setBookGenreData(await getDoc(doc(db, "genres", docSnap.data().genre))); // Get genre data
                    setRatingAverage(Object.values(docSnap.data().rating).reduce((acc, rating) => acc + parseInt(rating), 0) / Object.keys(docSnap.data().rating).length); // Calculate rating average
                    setRatingTimes(Object.keys(docSnap.data().rating).length); // Get rating times
                    setLoading(false);
                } else {
    ...
    const handleRatingChange = async (event) => {
        ...
        // Update book rating on book document
        const bookDocRef = doc(db, "books", bookId);
        const bookDocSnap = await getDoc(bookDocRef);
        const bookData = bookDocSnap.data();

        //Maintain the previous ratings and add the new user one or update it
            ...
            <div className="flex items-center mb-8">
                <button className="text-zinc-200" onClick={() => navigate(-1)}>
                    <div className="flex items-center gap-2">
                        <ArrowBackIosIcon />
                        <span className="text-2xl font-semibold ml-4 text-zinc-200">Back</span>
                    </div>
                </button>
            </div>
            ...
            {loading ? (
                ...
                <section className="flex flex-col sm:flex-row gap-10">
                    <div className="w-full sm:w-80 flex flex-col items-center gap-3 relative">
                        <img src={book.cover} alt={`Cover of ${book.title}`} className="rounded-lg" />
                        <button className='relative w-full'>
                            <div className="flex items-center p-3 cursor-pointer bg-carrot gap-4 justify-between w-full rounded-md" onClick={toggleAddToList}>
                                ...
                                <PlaylistAddRounded className="text-white" />
                            </div>
                            {showAddToList && (
                                <AddToList
                                    toggleAddToList={toggleAddToList}
                                    bookId={bookId}
                                    className="w-52 h-auto shadow p-5 rounded-lg z-10 absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2"
                                />
                            )}
                        </button>
                    ...
                    <div className="w-full md:w-full">
                        <div>
                            ...
                            <div>
                                <h3 className="text-lg font-normal mb-2">Synopsis</h3>
                                <p className="text-gray-500 overflow-x-scroll text-justify">{book.sipnosis}</p>
                            </div>
                            <article className="justify-between mx-16 gap-16 mt-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
                                <div className="flex flex-col items-center gap-2">
                                    <img src={bookIconImg} alt="Pages Icon" className='w-12' />
                                ...
                                <div className="flex flex-col items-center gap-2">
                                    <img src={editorialImg} alt="Editorial Icon" className='w-12' />
                                    <div className="text-center">
                                ...
                                <div className="flex flex-col items-center gap-2">
                                    <img src={languageIconImg} alt="Language Icon" className='w-12' />
                                    <div className="text-center">
                                ...
                                <div className="flex flex-col items-center gap-2">
                                    <img src={ibanCode} alt="ISBN Icon" className='w-12' />
                                    <div className="text-center">
